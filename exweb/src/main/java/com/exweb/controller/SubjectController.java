...
import com.exservice.service.impl.SubjectTreeImpl;
...
public class SubjectController {
    ...
    public String add(@PathVariable String parentid, Model model ){
        Subject parent = subjectManager.findSubject(parentid);
    ...
    public String subject_view(Model model , @PathVariable String subjectid){
        ...
        if(subject != null)
        {
            ...
            if(SystemConfig.root.equals(subject.getParentid()))
            {
                ...
                parent = subjectManager.findSubject(subject.getParentid());
            }
            model.addAttribute("parent",parent);
        }
    ...
    public MessageObject save(
        ...
        Subject sub = subjectManager.findSubject(parentid+"_"+subjectid);
        Subject parent = subjectManager.findSubject(parentid);
        if(parent == null){
            mo.setCode(SystemConfig.mess_failed);
    ...
    public List<Subject> viewlist(){
        List<Subject> list = (List<Subject>) subjectManager.findAll();
        return list;
    }
    ...
    public List<Subject> findSubjectBypid(@PathVariable String pid){
        List<Subject> list = (List<Subject>) subjectManager.findSubjectBypid(pid);
        return list;
    }
